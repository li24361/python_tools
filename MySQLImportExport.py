...
class MySQLImporter(object):
    def __init__(self, host=None, user=None, passwd=None, db=None, short=True, separator='|', filters=[], charset='utf8'):
        """
        ...
        if host is not None and user is not None and passwd is not None and db is not None:
            ...
            self.con.set_character_set(charset)
    ...
    def create_insert_query(self, item, is_insert=True):
        """
        ...
        if is_insert:
            ...
            self.query = 'REPLACE '
        self.query += ' INTO ' + self.table_name + ' ('
    ...
    def import_item(self, item, is_insert=True):
        """
        ...
        if rows.num_rows() != 0:
            ...
            self.create_insert_query(item, False)
            self.con.query(self.query)
...
class MySQLExporter(object):
    def __init__(self, host=None, user=None, passwd=None, db=None, charset='utf8'):
        """
           This constructor can initialize the connection to the database
    ...
    def _build_where_condition(self, conditions):
        ...
        if len(conditions) > 0:
            if type(conditions).__name__ == 'str':
